...
try:
    ...
    try:
        ...
        raise ImportError("Cannot import RecursiveCharacterTextSplitter.")

try:
    from langchain_community.embeddings import HuggingFaceEmbeddings
    from langchain_community.vectorstores import FAISS
except Exception:
    raise ImportError("Cannot import langchain_community components.")
...
try:
    ...
    try:
        ...
        raise ImportError("Cannot import Document/PromptTemplate.")
...
try:
    ...
    try:
        ...
        try:
            ...
            raise ImportError("Cannot import LLMChain or RunnableSequence.")
...
# -------------------------------------------------------------------------
# Compatibility shim for old/new LangChain
# -------------------------------------------------------------------------
def _make_llm_chain(llm, prompt):
    """Safely construct an LLMChain or Runnable-compatible object."""
    try:
        return LLMChain(llm=llm, prompt=prompt)
    except TypeError:
        if hasattr(LLMChain, "from_llm"):
            return LLMChain.from_llm(llm, prompt)
        return LLMChain(prompt | llm)
...
"""
    ...
    return chain.invoke({"context": context_text, "question": query})["text"] \
        if hasattr(chain, "invoke") else chain.run({"context": context_text, "question": query})
...
{doc}"""
    ...
    chain = _make_llm_chain(llm, prompt)
    return chain.invoke({"doc": text})["text"] if hasattr(chain, "invoke") else chain.run({"doc": text})
...
Respond in JSON:
{
  ...
  "Obligations": ["..."]
    ...
    prompt = PromptTemplate(template=prompt_template, input_variables=["doc"])
    chain = _make_llm_chain(llm, prompt)
    raw = chain.invoke({"doc": text[:8000]})["text"] if hasattr(chain, "invoke") else chain.run({"doc": text[:8000]})
    try:
        cleaned = raw.strip()
...
def safe_analyze_document_for_risks(text: str, model_name: str = None) -> Tuple[Dict[str, List[str]], List[str]]:
    try:
        ...
        print("⚠️ safe_analyze_document_for_risks fallback:", e)
